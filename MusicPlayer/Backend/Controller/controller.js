  ...
  try {
    ...
    if (newSongs.length === 0) {
      return res
        .status(400)
        .json({
          message: "All the provided songs are already in the playlist",
        });
    }
    ...
    return res
      .status(500)
      .json({ message: "Something went wrong while adding songs to playlist" });
  }
};
...
exports.removeSongsToPlaylist = async (req, res) => {
  ...
  try {
    ...
    if (newSongs.length === 0) {
      return res
        .status(400)
        .json({ message: "All the provided songs are not in the playlist" });
    }
    ...
    return res
      .status(500)
      .json({
        message: "Something went wrong while removing songs from playlist",
      });
  }
};
